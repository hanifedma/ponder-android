...
object Strings {
    ...
    private val EN_MAP: Map<String, String> = mapOf(
        ...
        "settings.notify" to "Thought in the notification bar",
        "settings.notify.hint" to "Keeps one of your entries in the notification shade, readable straight from the lock screen. Swipe it away and the next random one takes its place.",
        "settings.notify.source" to "Pick from",
        "settings.notify.source.hint" to "Which section the notification draws from.",
        "settings.notify.source.all" to "Both sections",
        "settings.notify.empty" to "Nothing to show yet — add an entry and it appears in the shade.",
        "settings.notify.blocked" to "Android is blocking Ponder's notifications, so nothing can be shown.",
        "settings.notify.open" to "Open notification settings",
        "settings.keepAlive" to "Keep running in the background",
        "settings.keepAlive.hint" to "Comes back on its own after the phone restarts, and survives clearing recent apps. Adds one small silent entry at the bottom of the shade.",
        "settings.battery" to "Battery",
        "settings.battery.hint" to "Android puts unused apps to sleep, which can stall the notification. Allowing background activity keeps it instant.",
        "settings.battery.ok" to "Background activity is allowed ✓",
        "settings.battery.ask" to "Allow background activity",
        "notify.channel.thoughts" to "Thought in the shade",
        "notify.channel.thoughts.desc" to "One of your quotes, thoughts or tips at a time. Swipe it away for the next one.",
        "notify.channel.keepAlive" to "Background service",
        "notify.channel.keepAlive.desc" to "Silent placeholder that lets Ponder keep running, so the thought comes back after a restart.",
        "notify.keepAlive.title" to "Ponder is running",
        "notify.keepAlive.text" to "Keeping a thought in your notification shade.",
        "widget.title" to "❝ Ponder",
        "widget.next" to "Show another",
        "widget.empty" to "Nothing saved here yet. Tap to add your first one.",
        "settings.widget" to "Home screen widget",
        "settings.widget.hint" to "Which section the widget picks from. Each widget keeps its own quote, so two of them show two different things.",
        "err.noScreen" to "This phone has no settings screen for that.",
    ...
    private val KO_MAP: Map<String, String> = mapOf(
        ...
        "settings.notify" to "알림창에 문구 표시",
        "settings.notify.hint" to "저장한 항목 하나를 알림창에 띄워 잠금화면에서도 바로 읽을 수 있습니다. 알림을 밀어서 지우면 다음 항목이 무작위로 나타납니다.",
        "settings.notify.source" to "표시할 섹션",
        "settings.notify.source.hint" to "알림에 어떤 섹션의 항목을 띄울지 정합니다.",
        "settings.notify.source.all" to "두 섹션 모두",
        "settings.notify.empty" to "아직 표시할 항목이 없습니다 — 항목을 추가하면 알림창에 나타납니다.",
        "settings.notify.blocked" to "Android가 Ponder의 알림을 차단하고 있어 아무것도 표시할 수 없습니다.",
        "settings.notify.open" to "알림 설정 열기",
        "settings.keepAlive" to "백그라운드에서 계속 실행",
        "settings.keepAlive.hint" to "휴대폰을 다시 켜면 스스로 시작하고, 최근 앱을 모두 지워도 유지됩니다. 알림창 아래에 작고 조용한 항목이 하나 추가됩니다.",
        "settings.battery" to "배터리",
        "settings.battery.hint" to "Android는 사용하지 않는 앱을 절전 상태로 두어 알림이 늦어질 수 있습니다. 백그라운드 활동을 허용하면 항상 즉시 동작합니다.",
        "settings.battery.ok" to "백그라운드 활동이 허용되었습니다 ✓",
        "settings.battery.ask" to "백그라운드 활동 허용",
        "notify.channel.thoughts" to "알림창의 문구",
        "notify.channel.thoughts.desc" to "저장한 명언·생각·건강 팁을 한 번에 하나씩 보여줍니다. 밀어서 지우면 다음 항목이 나옵니다.",
        "notify.channel.keepAlive" to "백그라운드 서비스",
        "notify.channel.keepAlive.desc" to "Ponder가 계속 실행되도록 하는 무음 알림입니다. 재시작 후에도 문구가 다시 나타납니다.",
        "notify.keepAlive.title" to "Ponder 실행 중",
        "notify.keepAlive.text" to "알림창에 문구를 유지하고 있습니다.",
        "widget.title" to "❝ Ponder",
        "widget.next" to "다른 항목 보기",
        "widget.empty" to "아직 저장된 항목이 없습니다. 눌러서 첫 항목을 추가하세요.",
        "settings.widget" to "홈 화면 위젯",
        "settings.widget.hint" to "위젯이 어떤 섹션에서 항목을 고를지 정합니다. 위젯마다 각자의 항목을 유지하므로 두 개를 두면 서로 다른 항목이 표시됩니다.",
        "err.noScreen" to "이 기기에는 해당 설정 화면이 없습니다.",
